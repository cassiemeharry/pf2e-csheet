...
impl RRefInner {
    ...
    fn new_from_name(name: impl Into<String>) -> Self {
        Self {
            resource: Arc::new(RwLock::new(ResourceRefName::Unresolved(name.into()))),
            modifier: None,
        }
    }

    #[inline]
    fn new_from_name_mod(name: impl Into<String>, modifier: impl Into<String>) -> Self {
        Self {
            resource: Arc::new(RwLock::new(ResourceRefName::Unresolved(name.into()))),
    ...
    #[inline]
    fn new_from_resolved_mod(resolved: Arc<Resource>, modifier: impl Into<String>) -> Self {
        Self {
            resource: Arc::new(RwLock::new(ResourceRefName::Resolved(resolved))),
            modifier: Some(modifier.into()),
        }
    ...
    fn from_str(s: &str) -> Result<RRefInner> {
        ...
        match mod_str_opt {
            ...
            Some(mod_str) => {
                ...
                Ok(RRefInner::new_from_name_mod(name_str, mod_str))
            }
        }
    }
}
...
#[derive(Clone, Eq, PartialEq, Hash, Ord, PartialOrd, Deserialize)]
#[serde(try_from = "smartstring::alias::String")]
...
impl ResourceRef {
    ...
    pub fn new_from_name_mod(name: impl Into<String>, modifier: impl Into<String>) -> Self {
        Self {
    ...
    pub fn new_from_resolved_mod(resolved: Arc<Resource>, modifier: impl Into<String>) -> Self {
        let expected_type = Some(resolved.get_type());
        Self {
...
impl fmt::Debug for ResourceRef {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.debug_struct("ResourceRef")
            .field("resource", &format_args!("{}", self.inner))
...
impl From<pf2e_csheet_shared::common::ResourceRef> for ResourceRef {
    fn from(common: pf2e_csheet_shared::common::ResourceRef) -> Self {
        let mut rref = match common.modifier {
            Some(m) => Self::new_from_name_mod(common.name, m),
            None => Self::new_from_name(common.name),
        };
        if let Some(rt) = common.resource_type {
            rref = rref.expect_type(rt);
        }
        rref
    }
}
...
impl<R: HasResourceType> TypedRef<R> {
    ...
    pub fn new_from_resolved(resolved: R) -> Self {
        let resolved = Arc::new(resolved.into());
        Self {
            inner: RRefInner::new_from_resolved(resolved),
            expected_type: PhantomData,
        }
    ...
    pub fn new_from_resolved_mod(resolved: R, modifier: impl Into<String>) -> Self {
        let resolved = Arc::new(resolved.into());
        Self {
            inner: RRefInner::new_from_resolved_mod(resolved, modifier),
            expected_type: PhantomData,
        ...
        D: de::Deserializer<'de>,
    {
            ...
            R: HasResourceType + de::Deserialize<'de>,
        {
                ...
                A: de::MapAccess<'de>,
            {
                ...
                let inner_resource: R = <R as de::Deserialize>::deserialize(mad)?;
                Ok(TypedRef::new_from_resolved(inner_resource))
            }
        }
